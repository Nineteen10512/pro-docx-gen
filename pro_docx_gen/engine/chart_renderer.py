...
def _label_density(categories: list, axis_size: float, width_in: float) -> float:
    """Estimate whether category labels will collide on a horizontal axis."""
    if not categories or width_in <= 0:
        return 0.0
    total_chars = sum(len(str(c)) for c in categories)
    return (total_chars * max(axis_size, 1.0)) / (width_in * 72.0)


def _should_auto_bar(chart_type: str, categories: list, series: list, axis_size: float, width_in: float) -> bool:
    """Prefer horizontal bars when long categorical labels would make columns unreadable."""
    if chart_type != "column" or len(series) != 1 or len(categories) < 4:
        return False
    longest = max((len(str(c)) for c in categories), default=0)
    return longest >= 16 or _label_density(categories, axis_size, width_in) >= 0.72


def _wrap_category_labels(categories: list, max_chars: int = 14) -> list[str]:
    labels = []
    for item in categories:
        ...
        if len(text) <= max_chars:
            ...
            labels.append("\n".join(textwrap.wrap(text, width=max_chars, break_long_words=False) or [text]))
    return labels


def _baseline_array(base, length: int):
    """Return one baseline per bar; scalar baselines must not drop labels after the first bar."""
    arr = np.atleast_1d(base).astype(float)
    if len(arr) == 1 and length > 1:
        return np.repeat(arr[0], length)
    return arr
...
def render_chart_to_png(
    ...
    output_path: str,
    dpi: int | None = None,
    available_height_inches: float | None = None,
    ...
    Args:
        ...
        available_height_inches: 渲染器传入的当前页剩余可用高度（英寸）。
            当 ``figure_height > available_height_inches * 0.9`` 时，渲染器
            会按比例自动缩图（保持宽高比），避免图表被推到下一页并留下
            一张近似空白页。默认 ``None`` 表示不启用缩图，保留 v1.6.0
            行为以保证向后兼容。
        compact_mode: 紧凑模式开关。``True`` 时图表前后不增加额外段间距
            （``space_before=0, space_after=0``），caption 段落被绑定到下
            一段（``keep_with_next=True``），用于把多张紧凑图连插在一节
            内（KPI 卡片墙/对比矩阵等场景）。默认 ``False``。

    Returns:
        输出文件路径（同 output_path）。同时通过 ``chart_spec`` 写入运行时
        元数据 ``_figure_height_in`` / ``_compact_mode``，供 renderer.py
        决定段落格式。
    """
    _require_matplotlib()
    _configure_fonts()
    ...
    # Pre-insertion legibility contract: the smallest rendered chart text must
    # remain at least 9pt at the final DOCX display size.
    axis_size = max(_pt(font_tok["size"].get("chart_axis"), 9), 10)
    legend_size = max(_pt(font_tok["size"].get("chart_legend"), 10), 10)
    title_size = max(_pt(font_tok["size"].get("chart_title"), 11), 12)
    ...
    # 饼图默认显示数据标签（百分比）；单系列柱/条图也默认标数值，避免比较图信息缺失。
    if show_data_labels is None:
        show_data_labels = chart_type in ("pie", "doughnut") or (
            chart_type in ("column", "bar") and len(series) == 1 and len(categories) <= 12
        )
    ...
    # v1.6.6: page-fit contract — if the figure would overflow the remaining
    # page area, scale it down proportionally (keeping aspect ratio) so the
    # chart + caption + small gap fit on one page. This is the root-cause
    # fix for the "stray blank page after a tall chart" regression: the
    # original code only set space_before/after and never measured how much
    # page was left, so a 6.27×4.7in chart at the bottom of a half-filled
    # page would push itself to the next page.
    fill_ratio = 0.9
    scale_applied = 1.0
    if available_height_inches is not None and available_height_inches > 0:
        max_h = float(available_height_inches) * fill_ratio
        if img_h > max_h and max_h > 0:
            scale_applied = max_h / img_h
            img_h = max_h
            img_w = img_w * scale_applied
    # Stash the final figure height (in inches) and compact_mode onto the
    # chart_spec so renderer.py can apply matching paragraph spacing /
    # keep_with_next for the caption. We mutate the spec in place because
    # renderer.py still holds the same reference.
    chart_spec["_figure_height_in"] = round(float(img_h), 3)
    chart_spec["_figure_width_in"] = round(float(img_w), 3)
    chart_spec["_figure_scale_applied"] = round(float(scale_applied), 4)
    chart_spec["_compact_mode"] = bool(compact_mode)

    if chart_spec.get("auto_orientation", True) and _should_auto_bar(chart_type, categories, series, axis_size, img_w):
        chart_type = "bar"
...
def _draw_column(ax, categories, series, palette, stacked, bar_width,
    ...
    ax.set_xticks(x)
    labels = _wrap_category_labels(categories)
    rotation = 35 if _label_density(categories, axis_size, max(ax.figure.get_figwidth(), 1.0)) >= 0.55 else 0
    ax.set_xticklabels(
        labels,
        fontsize=max(axis_size - (1 if rotation else 0), 7),
        color=text_color,
        rotation=rotation,
...
def _add_bar_labels(ax, bars, vals, bottom, text_color, axis_size):
    bases = _baseline_array(bottom, len(vals))
    for bar, v, b in zip(bars, vals, bases):
...
def _add_barh_labels(ax, bars, vals, left, text_color, axis_size):
    bases = _baseline_array(left, len(vals))
    for bar, v, l in zip(bars, vals, bases):
