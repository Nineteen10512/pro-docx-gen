...
def _apply_template(doc: dict, template_name: Optional[str]) -> dict:
    ...
    # v1.5.1: 确保 doc 至少有最小 meta（title），否则 parser 校验失败
...
def _check_docx_template_match(
    ...
    try:
        try:
            from .shared.quality import contrast_ratio, hex_to_rgb
        except ImportError:
            from .shared.color_palette import hex_to_rgb
            # 自带简化版 contrast_ratio
            def _lum(rgb):
                def _chan(c):
                    c = c / 255.0
...
def _auto_resolve_docx_variant(doc: dict, lang: str = "en") -> str:
    """v1.6.0: 根据文档内容自动推断最佳 variant。

    策略：
    1. 从 meta/scene 字段提取场景关键词
    2. 从 sections 标题提取内容关键词
    3. 用 match_variant 匹配最佳变体
    """
    from .v160_style_registry import match_variant as _match
    scene = ""
    keywords_parts = []
    if isinstance(doc, dict):
        ...
        for sec in sections[:5]:
            if isinstance(sec, dict):
                keywords_parts.append(sec.get("title", ""))
    keywords = " ".join(keywords_parts)
    return _match(scene=scene, keywords=keywords)
...
def generate(
    ...
    Args:
        content: 语义 JSON 字典，或 Markdown 字符串。v1.5.1 起允许 None（搭配 template_name）。
        ...
        template_name: v1.5.1 新增。可选 DOCX 模板名（academic/business/thesis_full/...）；
            命中注册表时，用模板的 default_structure 填充 doc 缺失字段，浅合并 theme_overrides。
            不传或未命中时与 v1.5 行为一致（字节级兼容）。
        template_path: v1.5.1 新增。可选 .docx/.dotx/.wpt 文件路径；
            从该文件提取主题（色板/字体）合并到 tokens。优先于 template_name 的 theme_overrides。
        auto_taste_match: v1.5.1 新增。True 时执行模板匹配评分（对比度+字体完整性+一致性），
            低分（<60）时给 warning（不阻断生成）。
        variant: v1.6.0 新增。显式指定风格变体（corporate_formal/minimal_clean/modern_tech/
            bold_impact/elegant_luxury），覆盖 auto_style 的自动推断。
        auto_style: v1.6.0 新增。True 时自动推断最佳 variant + theme，用户不传参数也能出合理风格。
    ...
    # v1.5.1: 模板注入（在 output_path 和 effective_theme 计算之前）
    if template_name:
    ...
    if isinstance(effective_theme, dict):
        # v1.5.1: 用户传 hex str 时转 RGBColor（merge_theme 不做类型转换）
    ...
    # v1.5.1: A 能力域 — template_path 提取并合并到 tokens
    _extracted_for_match: Optional[dict] = None
    if template_path:
        tokens, _extracted_for_match = _extract_and_apply_template_path(tokens, template_path)

    # v1.6.0: variant / auto_style — 风格变体注入
    _resolved_variant = variant
    if auto_style and not variant:
        _resolved_variant = _auto_resolve_docx_variant(doc, lang)
    if _resolved_variant:
        from .variant_tokens import apply_variant as _apply_variant
        tokens = _apply_variant(tokens, _resolved_variant)

    # v1.5.1: A 能力域 — auto_taste_match 检查
    if auto_taste_match:
    ...
    renderer = DocxRenderer(tokens, lang=lang, variant=_resolved_variant)
...
def generate_with_collaboration(
    ...
    """生成 docx 并立即开启协作会话（v1.5.1 C 能力域快捷入口）。
