...
def _rgb_to_hex_hash(color) -> str:
    """将 docx RGBColor 或 tuple 转为 '#RRGGBB' 形式（带 # 前缀，供 matplotlib 使用）。"""
    from shared.color_palette import rgb_to_hex as _r2h
    return "#" + _r2h(color, "333333")
...
def render_chart_to_png(
    ...
    palette = [_rgb_to_hex_hash(c) for c in chart_tok.get("palette", [])]
    if not palette:
        palette = ["#1F3864", "#C0504D", "#2E75B6", "#7F604F"]

    grid_color = _rgb_to_hex_hash(chart_tok.get("gridline_color", color_tok.get("muted", (0xDD, 0xDD, 0xDD))))
    text_color = _rgb_to_hex_hash(chart_tok.get("text_color", color_tok.get("text", (0x33, 0x33, 0x33))))
