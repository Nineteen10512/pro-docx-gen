"""merge — DOCX 协作合并与导出 API（v1.5.3 C 能力域）。
...
def export_review_pdf(
    ...
    """Export a review PDF, preferring docx2pdf and falling back to to_pdf()."""
    if not os.path.exists(session.docx_path):
        raise FileNotFoundError(session.docx_path)
    if not output_dir:
        raise ValueError("output_dir 不能为空")
    ...
    try:
        ...
        from ..docx_jsx import to_pdf as _to_pdf

        return _to_pdf(session.docx_path, output_dir=output_dir)
