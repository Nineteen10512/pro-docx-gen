...
4. Run `assess_translation_risk(docx_path, translations)` before final write-back when table content expands materially.
5. Write translated text back with `apply_translation_map(docx_path, translations, output_path=..., target_lang=..., auto_format_tables=True)`.
5. Keep images, tables, headers, footers, and nearby semantic placement intact. Do not move charts or figures unless overflow makes the file unreadable.
6. If the target language expands significantly, tighten wording before changing structure. Prefer shorter faithful phrasing over reflowing the entire page.
...
# PRO-DOCX v1.5.1 — PaperJSX 专业 Word 文档生成
