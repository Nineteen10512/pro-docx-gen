---
name: pro-docx-gen
description: Use when users need to create, edit, or translate DOCX/Word documents, reports, papers, lesson plans, references, TOC, footnotes, comments, track changes, equations, cross-references, multicolumn layout, PDF/image/text extraction, or Markdown-to-DOCX output with PRO-DOCX semantic JSON. Also use when the task is to preserve the original file's layout, images, and nearby information placement while translating Chinese to English, English to Chinese, or bilingualizing an existing DOCX file.
---

## Translation Workflow

Use structure-preserving translation when the user wants an existing DOCX file translated with minimal visual change.

1. Treat the source DOCX as the visual source of truth. Prefer editing a copy of the original file over regenerating from scratch.
2. Collect translatable segments with `collect_translation_segments(docx_path)`.
3. Translate only the returned text payloads. Preserve numbers, dates, units, citations, formula syntax, placeholders, figure numbers, and brand names unless the user explicitly asks to localize them.
4. Run a self-audit before final write-back: translated output must not add any fact, example, explanation, qualifier, summary, inferred intent, or background detail that does not already exist in the source file. If the source is ambiguous, keep the ambiguity or translate conservatively; do not "fill in" missing information.
5. Run `assess_translation_risk(docx_path, translations)` before final write-back when table content expands materially.
6. Write translated text back with `apply_translation_map(docx_path, translations, output_path=..., target_lang=..., auto_format_tables=True)`.
7. Keep images, tables, headers, footers, and nearby semantic placement intact. Do not move charts or figures unless overflow makes the file unreadable.
8. If the target language expands significantly, tighten wording before changing structure. Prefer shorter faithful phrasing over reflowing the entire page.

Table translation layer:
- Translate table cells normally. Do not skip tables.
- Preserve row/column meaning exactly, even when the target text is longer.
- Let the engine judge layout risk from cell growth ratio and column density.
- For medium risk, allow mild font shrink and table autofit.
- For high risk, allow stronger autofit plus proportionate table-level tightening, but do not paraphrase away key meaning just to save space.

Recommended modes:
- Replace mode: replace source-language text in place.
- Bilingual mode: keep the original and insert the translation immediately after the original paragraph or cell when the user asks for side-by-side readability.
- Review mode: use comments or track changes when the user wants translation review instead of a clean final file.

# PRO-DOCX v1.6.0 — PaperJSX 专业 Word 文档生成与文件翻译
...
## 版本亮点 v1.6.0（2026-07）
- **结构保真翻译工作流**：新增 `collect_translation_segments()`、`apply_translation_map()`、`assess_translation_risk()`，支持现有 DOCX 在尽量不改排版的前提下进行中英互译。
- **图片保真正式修复**：翻译回写时保护图片、域代码、脚注/尾注引用与其他非纯文本节点，避免“翻译后图片丢失”。
- **表格翻译层**：表格内容可直接翻译；引擎会根据风险自动执行 mild shrink、autofit 或更强的表格收紧策略。
- **5 套全局风格变体**：`corporate_formal`、`minimal_clean`、`modern_tech`、`bold_impact`、`elegant_luxury`。
- **DOCX 审美预检**：新增 `taste_check()`，可在生成前识别密度、气质和可读性风险。
- **默认 smoke 覆盖图片保真**：常规冒烟测试已纳入 `DOCX image preservation` 与跨包 `image regression`。
...
该脚本会检查技能 frontmatter、公开导入、PPTX/DOCX 语义生成、Markdown 生成、SVG 迷你图和共享参考文献格式化。若要保留生成文件用于检查，可加 `--output-dir smoke_tests/_output`。DOCX 图表 PNG 测试依赖 `matplotlib/numpy`，缺失时会显示 SKIP，不影响基础生成检查。
...
质量门会检查 DOCX 包结构、核心 XML、可见文本、占位符文本、超长文本和基础表格几何；检测到 LibreOffice/`soffice` 时会尝试导出 PDF 作为视觉 QA 辅助。若需要把警告也视为失败，可加 `--strict`。
...
*版本：v1.6.0 | 架构：PaperJSX 语义编译 | Design Tokens 驱动 | 确定性布局*
